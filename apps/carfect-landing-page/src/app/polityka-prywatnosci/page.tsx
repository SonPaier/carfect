...
export async function generateMetadata(): Promise<Metadata> {
  const page: LegalPage | null = await client.fetch(legalPageBySlugQuery, { slug: 'polityka-prywatnosci' }, { next: { tags: ['sanity'] } });

  return {
    title: page?.seo?.metaTitle || 'Polityka Prywatności',
    description: page?.seo?.metaDescription || 'Polityka prywatności serwisu Carfect.pl.',
...
export default async function Page() {
  const [page, settings] = await Promise.all([
    client.fetch<LegalPage | null>(legalPageBySlugQuery, { slug: 'polityka-prywatnosci' }, { next: { tags: ['sanity'] } }),
    client.fetch<SiteSettings | null>(siteSettingsQuery, {}, { next: { tags: ['settings'] } }),
  ]);

  // If no Sanity content yet, fall back to the old component
  if (!page) {
    const PrivacyPolicy = (await import('@/components/pages/PrivacyPolicy')).default;
    return (
      <>
        <Breadcrumbs items={[
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 pt-24">
        <Breadcrumbs items={[
          { name: 'Strona główna', href: '/' },
          { name: page.title, href: '/polityka-prywatnosci' },
        ]} />
