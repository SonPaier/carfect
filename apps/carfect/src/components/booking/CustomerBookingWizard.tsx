...
export default function CustomerBookingWizard({
  ...
  const existingReservation = location.state?.existingReservation as
    | ExistingReservation
    | undefined;
    ...
    if (step === 'datetime' && selectedService && instance?.working_hours && stations.length > 0) {
      ...
      for (let i = 0; i < daysAhead && !foundFirstSlot; i++) {
        ...
        for (
          ...
          const timeStr = `${Math.floor(time / 60)
            .toString()
          ...
          const availableStation = compatibleStations.find((station) => {
            ...
            return !stationBlocks.some((block) => {
              const blockStart =
                parseInt(block.start_time.split(':')[0]) * 60 +
                parseInt(block.start_time.split(':')[1]);
              const blockEnd =
                parseInt(block.end_time.split(':')[0]) * 60 +
                parseInt(block.end_time.split(':')[1]);
    ...
    const fetchData = async () => {
      ...
      if (instanceData) {
        const parsedInstance: Instance = {
          ...
          working_hours: instanceData.working_hours as Record<
            string,
            {
              open: string;
              close: string;
        ...
        const { data: servicesData } = (await supabase
          ...
          .order('sort_order')) as unknown as { data: Service[] | null };
        ...
        const { data: blocksData } = await supabase.rpc('get_availability_blocks', {
  ...
  const popularServices = services
    .filter((s) => POPULAR_KEYWORDS.some((k) => s.name.toLowerCase().includes(k)))
    .slice(0, 3);
        ...
        for (const station of compatibleStations) {
          ...
          const hasConflict = stationBlocks.some((block) => {
            const blockStart =
              ...
              parseInt(block.start_time.split(':')[1]);
            const blockEnd =
              parseInt(block.end_time.split(':')[0]) * 60 + parseInt(block.end_time.split(':')[1]);
            return time < blockEnd && endTime > blockStart;
          });
          if (!hasConflict) {
  ...
  const getSlotsCountByTimeOfDay = (
    slots: TimeSlot[],
  ): {
    morning: number;
    afternoon: number;
  ...
  if (step === 'phone') {
    return (
      <div
        className={cn(
          'pb-16',
          slideDirection === 'forward' ? 'animate-slide-in-left' : 'animate-slide-in-right',
        )}
      >
        <section className="py-4 md:py-6 text-center">
          <div className="container">
            <h1 className="text-lg md:text-xl font-bold text-foreground">{t('booking.title')}</h1>
          </div>
        </section>
            ...
            <div className="bg-white border border-border p-4 space-y-4">
              <div>
                ...
                {isVerifiedCustomer && customerPhone.length >= 9 && (
                  <p className="text-green-600 mt-1.5 flex items-center gap-1 text-base">
              ...
              {customerPhone.length >= 9 && (
                <div className="animate-fade-in">
                  <Label htmlFor="carModel" className="text-base font-medium">
                  ...
                  {historicalCarModels.length > 1 && (
                    <div className="flex flex-wrap gap-2 mt-2 py-[8px]">
              ...
              <Button
                onClick={() => goToStep('service', 'forward')}
                className="w-full"
  ...
  if (step === 'service') {
    ...
    const renderServiceItem = (service: Service) => {
      ...
      return (
        <button
          ...
          className={cn(
            'bg-white border border-border p-3 text-left transition-all w-full',
        ...
        >
          <div className="grid grid-cols-[24px,1fr,auto] items-start gap-3">
            ...
            <div className="min-w-0 overflow-hidden">
              ...
              {service.description && (
                <ServiceDescription description={service.description} serviceId={service.id} />
              )}
    ...
    return (
      <div className="min-h-screen bg-background pb-16 overflow-x-hidden">
        <div
          className={cn(
            'px-4 py-4 max-w-[550px] mx-auto w-full',
            slideDirection === 'forward' ? 'animate-slide-in-left' : 'animate-slide-in-right',
          )}
        >
          <button
            onClick={() => goToStep('phone', 'back')}
            className="flex items-center gap-1 text-muted-foreground hover:text-foreground mb-4 text-base"
          >
          ...
          <div className="grid gap-2 mb-3">{services.map(renderServiceItem)}</div>
  ...
  if (step === 'datetime') {
    ...
    const selectedDayData = selectedDate
      ? availableDays.find((d) => isSameDay(d.date, selectedDate))
      : null;
    ...
    return (
      <div className="min-h-screen bg-background pb-16">
        <div
          className={cn(
            'container py-4 max-w-[550px] mx-auto',
            slideDirection === 'forward' ? 'animate-slide-in-left' : 'animate-slide-in-right',
          )}
        >
          ...
          <div className="flex items-center justify-between px-2 mb-4">
            ...
            <button
              ...
              className="p-2 rounded-full hover:bg-hover text-muted-foreground transition-colors"
            >
    ...
    return (
      <div className="min-h-screen bg-background pb-16">
        ...
        >
          ...
          <div className="bg-white border border-border p-3 mb-3 space-y-1.5 text-sm">
            ...
            <div className="flex justify-between items-start">
              ...
              <div className="text-right">
                ...
                {selectedAddons.map((addonId) => {
                  const addon = services.find((s) => s.id === addonId);
                  return addon ? (
                    <span key={addonId} className="font-medium text-base block">
                      {addon.name}
                    </span>
                  ) : null;
                })}
            ...
            <p className="text-muted-foreground text-sm">* {t('booking.priceDisclaimer')}</p>
          </div>
          ...
          {editMode ? (
            ...
            <>
              ...
              <div className="bg-white border border-border p-3 mb-3 space-y-3">
              ...
              <Collapsible open={showNotes} onOpenChange={setShowNotes} className="mb-3">
                <CollapsibleTrigger className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors px-0 py-[8px]">
                  ...
                  <span className="text-base">
                    {t('booking.additionalNotes')} ({t('common.optional')})
                  </span>
                </CollapsibleTrigger>
              ...
              {!smsSent ? (
                    ...
                    <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
                      ...
                      <div className="space-y-4 text-sm">
                        ...
                        <ul className="list-disc pl-5 space-y-2">
                          <li>
                            <strong>{t('booking.rodoPartner')}</strong> {instance?.name}
                            {instance?.address ? `, ${instance.address}` : ''} –{' '}
                            {t('booking.rodoPartnerPurpose')}
                          </li>
                          <li>
                            <strong>{t('booking.rodoOperator')}</strong> Tomasz Nastały Sinpai, ul.
                            Prezydenta Lecha Kaczyńskiego 31 lok. 19, 81-810 Gdańsk, NIP 5851474597
                            – {t('booking.rodoOperatorPurpose')}
                          </li>
                        </ul>
                        ...
                        <ul className="list-disc pl-5 space-y-2">
                          <li>
                            <strong>{t('booking.rodoPurposeReservation')}</strong>{' '}
                            {t('booking.rodoPurposeReservationDesc')}
                          </li>
                          <li>
                            <strong>{t('booking.rodoPurposeContact')}</strong>{' '}
                            {t('booking.rodoPurposeContactDesc')}
                          </li>
                          <li>
                            <strong>{t('booking.rodoPurposeMarketing')}</strong>{' '}
                            {t('booking.rodoPurposeMarketingDesc')}
                          </li>
                        </ul>
                      </div>
                    </DialogContent>
                ...
                <div className="bg-white border border-border p-4 text-center">
                  <p className="text-xs text-muted-foreground mb-3">{t('booking.enterSmsCode')}</p>
  ...
  if (step === 'success' && confirmationData) {
    ...
    return (
      <div className="min-h-screen bg-background pb-16">
        <div
          className={cn(
            'container py-6 max-w-[550px] mx-auto',
            slideDirection === 'forward' ? 'animate-slide-in-left' : 'animate-slide-in-right',
          )}
        >
          <div className="max-w-sm mx-auto text-center">
            ...
            <div className="bg-white border border-border p-3 mb-4 text-left space-y-1.5">
            ...
            {instance?.auto_confirm_reservations && (
              <div className="bg-white border border-border p-3 mb-4 text-xs text-muted-foreground">
            ...
            {socialLinks.instagram && (
              <a
                ...
                className="bg-white border border-border p-4 flex items-center gap-3 hover:border-pink-500/50 transition-all group"
              >
