...
export const SummaryStepV2 = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // TODO: regenerate Supabase types to include visibility column
              ...
              onConfirm={(products) => {
                setServices((prev) =>
                  prev.map((s) => {
                    ...
                    const kept = s.selectedProducts.filter((p) =>
                      newSelectedIds.has(p.scopeProductId),
                    );
                ...
                onConfirm={(products) => {
                  setServices((prev) =>
                    prev.map((s) => {
                      ...
                      const kept = s.suggestedProducts.filter((p) =>
                        newSelectedIds.has(p.scopeProductId),
                      );
                      const keptIds = new Set(kept.map((p) => p.scopeProductId));
