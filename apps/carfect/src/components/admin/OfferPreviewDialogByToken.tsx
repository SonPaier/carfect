...
export function OfferPreviewDialogByToken({
  ...
  const fetchOfferData = useCallback(async () => {
    ...
    try {
      ...
      if (offerError || !offer) {
        setError(t('sendEmail.offerNotFound'));
        return;
      }
      ...
      if (offer.has_unified_services && enrichedOptions.length > 0) {
        const productIds = enrichedOptions.flatMap(
          (opt) => opt.offer_option_items?.map((item) => item.product_id).filter(Boolean) || [],
        );
        ...
        if (productIds.length > 0) {
          ...
          enrichedOptions = enrichedOptions.map((opt) => ({
            ...
            offer_option_items: opt.offer_option_items?.map((item) => ({
              ...item,
              unified_services:
                item.product_id && descMap[item.product_id]
      ...
      setError(t('sendEmail.loadError'));
    } finally {
      setLoading(false);
    }
  }, [token, t]);
