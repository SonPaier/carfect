...
const DashboardOverview = ({
  ...
  const workingDaysForFetch = useMemo(
  ...
  const fetchDateEnd =
    workingDaysForFetch[workingDaysForFetch.length - 1] || format(new Date(), 'yyyy-MM-dd');
  ...
  const fetchData = useCallback(async () => {
    ...
    const selectFields =
      'id, title, customer_name, customer_phone, item_date, end_date, start_time, end_time, status, column_id, customer_address_id, assigned_employee_ids, payment_status, price, priority';

    const [itemsRes, pendingRes, paymentItemsRes, remindersRes, overdueInvoicesRes] =
      await Promise.all([
        // Scheduled items in date range
        supabase
          .from('calendar_items')
          .select(selectFields)
          ...
          .order('item_date')
          .order('start_time'),
        // Pending items from the past (in_progress, unfinished, follow_up)
        supabase
          .from('calendar_items')
          .select(selectFields)
          .eq('instance_id', instanceId)
    ...
    // Merge scheduled + pending items, deduplicate
    ...
    const overdueItemIds = [...overdueMap.keys()].filter(
      (id) => !payItems.find((p) => p.id === id),
    );
    ...
    const addressIds = [
      ...new Set(
        allItemsForResolve.filter((i) => i.customer_address_id).map((i) => i.customer_address_id!),
      ),
    ];
    ...
    const allEmpIds = [
      ...new Set(allItemsForResolve.flatMap((i) => i.assigned_employee_ids || [])),
    ];
    ...
    // Sort: pending items from past float to top, then by date
    calItems.sort((a, b) => {
      const aIsPending =
        a.item_date < today && ['in_progress', 'unfinished', 'follow_up'].includes(a.status);
      const bIsPending =
        b.item_date < today && ['in_progress', 'unfinished', 'follow_up'].includes(b.status);
      ...
      <div className={`grid grid-cols-1 ${gridCols} gap-6`}>
        {settings.visibleSections.orders && (
          <DashboardColumn
            ...
            emptyText="Brak zleceń na najbliższe dni robocze"
          >
                      ...
                      <div>
                        ...
                        {prioritiesEnabled &&
                          ...
                          (() => {
                            ...
                            return (
                              <Badge className={`text-[11px] px-2 py-0.5 border ${cfg.badgeCls}`}>
                                {cfg.label}
                              </Badge>
                            );
                          })()}
          ...
          >
            {todayReminders.map((r, idx) => (
              <ReminderCard
                ...
                reminder={r}
                isFirst={idx === 0}
                onDone={(e) => handleReminderDone(r.id, e)}
        ...
        {settings.visibleSections.payments && (
          <DashboardColumn
            ...
            title="Płatności"
            count={allPaymentItems.length}
            emptyText="Brak płatności do rozliczenia"
          >
...
function getReminderUrgency(deadline: string): { badge: string; label: string | null } {
  ...
  if (days <= 7)
    return { badge: 'bg-yellow-100 text-yellow-700 border border-yellow-200', label: 'WKRÓTCE' };
  ...
  return (
    ...
    >
      <div className="flex items-start gap-2">
        <Checkbox className="mt-0.5 shrink-0" onClick={onDone} />
        <div className="space-y-1 min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2">
            <span className="font-medium text-sm leading-tight">{reminder.name}</span>
            {urgency.label && (
              <span
                className={`inline-flex items-center rounded-full px-1.5 py-0.5 text-[10px] font-semibold shrink-0 ${urgency.badge}`}
              >
                {urgency.label}
              </span>
            )}
...
const PaymentCard = ({
  ...
  >
    <div className="space-y-1">
      ...
      {item.customer_name && <div className="text-sm text-foreground">{item.customer_name}</div>}
      {item.price != null && (
        <div className="text-sm font-medium text-foreground">{item.price.toFixed(2)} PLN</div>
      )}
    </div>
  </div>
);
