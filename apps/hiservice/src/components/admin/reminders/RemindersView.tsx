...
import { Bell, Check, Plus, Settings2, Trash2 } from 'lucide-react';
...
function getRecurringLabel(recurringType: string | null, recurringValue: number | null): string {
  ...
  if (recurringType === 'monthly' && recurringValue)
    return `Cykliczne: ${recurringValue} dnia miesiąca`;
  if (recurringType === 'weekly' && recurringValue !== null)
    return `Cykliczne: co ${DAYS_OF_WEEK_LABELS[recurringValue] || ''}`;
  return 'Cykliczne';
}
...
function getUrgencyClasses(deadline: string): {
  border: string;
  text: string;
  badge: string;
  label: string | null;
...
export default function RemindersView({ instanceId }: Props) {
  const { reminders, loading, saveReminder, updateStatus, deleteReminder } =
  ...
  const archiveReminders = useMemo(
    () => reminders.filter((r) => r.status === 'done' || r.status === 'cancelled'),
    [reminders],
  );
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="flex border-b border-border/50">
        ...
        >
          Do zrobienia
          <span className="ml-2 text-xs px-1.5 py-0.5 rounded-full bg-white">
            {todoReminders.length}
          </span>
        </button>
        <button
      ...
      <div className="space-y-2">
        ...
        {!loading && displayedReminders.length === 0 && (
          <EmptyState
            ...
            description="Nie masz żadnych przypomnień."
          />
        )}
                ...
                {isArchive ? (
                  ...
                  <>
                    <div className="text-right">
                      <div className={`text-sm font-semibold ${urgency.text}`}>
                        {formatDeadline(r.deadline)}
                      </div>
                      <div className={`text-xs ${urgency.text}`}>{getDaysLabel(r.deadline)}</div>
                      {urgency.label && (
                        <Badge className={`text-[10px] mt-1 ${urgency.badge}`}>
                          {urgency.label}
                        </Badge>
                      )}
                    </div>
