...
const AddEditEmployeeDialog = ({
  ...
  const handleDelete = async () => {
    if (!employee) return;
    try {
      await updateEmployee.mutateAsync({ id: employee.id, active: false, name: employee.name });
        ...
        <DialogContent className="sm:max-w-md">
              ...
              <>
                <div className="flex items-center space-x-2">
                  ...
                  <Label htmlFor="createAccount" className="text-sm font-normal cursor-pointer">
                    Stwórz konto w aplikacji dla pracownika
                  </Label>
                </div>
                {createAccount && (
          ...
          <DialogFooter className="flex flex-row gap-2">
            ...
            <Button
              ...
              disabled={isSubmitting || !isAccountValid}
              className={cn(!isEditing && 'flex-1')}
            >
