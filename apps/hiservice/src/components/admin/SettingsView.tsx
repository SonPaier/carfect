...
const SettingsView = ({ instanceId, onColumnsChange }: SettingsViewProps) => {
  ...
  } = useInstanceFeature(instanceId, 'activities');
  const {
    enabled: employeesEnabled,
    loading: employeesLoading,
    toggle: toggleEmployees,
  } = useInstanceFeature(instanceId, 'employees');
  const {
    enabled: protocolsEnabled,
    loading: protocolsLoading,
    toggle: toggleProtocols,
  } = useInstanceFeature(instanceId, 'protocols');
  const {
    enabled: remindersEnabled,
    loading: remindersLoading,
    toggle: toggleReminders,
  } = useInstanceFeature(instanceId, 'reminders');
  const {
    enabled: prioritiesEnabled,
    loading: prioritiesLoading,
    toggle: togglePriorities,
  } = useInstanceFeature(instanceId, 'priorities');
  const {
    enabled: employeeCalendarViewEnabled,
    loading: employeeCalendarViewLoading,
    toggle: toggleEmployeeCalendarView,
  } = useInstanceFeature(instanceId, 'employee_calendar_view');
  const {
    enabled: projectsEnabled,
    loading: projectsLoading,
    toggle: toggleProjects,
  } = useInstanceFeature(instanceId, 'projects');
  ...
  const allTabs: { key: SettingsTab; label: string; icon: React.ReactNode }[] = [
    ...
    {
      key: 'employee-calendars',
      label: 'Kalendarze pracowników',
          ...
          <div className="bg-card rounded-lg border border-border shadow-sm p-6 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <AddressSearchInput
                ...
                defaultValue={
                  [
                    companyForm.address_street,
                    companyForm.address_postal_code,
                    companyForm.address_city,
                  ]
                    .filter(Boolean)
                    .join(', ') ||
                  companyForm.address ||
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                type="tel"
                className="bg-white"
                value={companyForm.blik_phone}
                onChange={(e) => handleInputChange('blik_phone', e.target.value)}
          ...
          <div className="bg-card rounded-lg border border-border shadow-sm p-6 space-y-6">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Włączaj i wyłączaj funkcjonalności dostępne w panelu.
              </p>
            </div>
            <div className="flex items-center justify-between py-3 border-t border-border">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">
                  Moduł powiadomień i aktywności w aplikacji
                </p>
              </div>
              <Switch
            ...
            <div className="flex items-center justify-between py-3 border-t border-border">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">
                  Moduł zarządzania pracownikami i przypisywania do zleceń
                </p>
              </div>
              <Switch
            ...
            <div className="flex items-center justify-between py-3 border-t border-border">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">
                  Moduł protokołów serwisowych zakończenia prac
                </p>
              </div>
              <Switch
            ...
            <div className="flex items-center justify-between py-3 border-t border-border">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">
                  Moduł przypomnień i śledzenia terminów
                </p>
              </div>
              <Switch
            ...
            <div className="flex items-center justify-between py-3 border-t border-border">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">
                  Priorytetyzacja zleceń (krytyczny, wysoki, normalny, niski)
                </p>
              </div>
              <Switch
            ...
            {employeesEnabled && (
              <div className="flex items-center justify-between py-3 border-t border-border">
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Widok kalendarza z podziałem na pracowników i detekcją konfliktów
                  </p>
                </div>
                <Switch
            ...
            <div className="flex items-center justify-between py-3 border-t border-border">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">
                  Grupuj zlecenia w wieloetapowe projekty
                </p>
              </div>
              <Switch
