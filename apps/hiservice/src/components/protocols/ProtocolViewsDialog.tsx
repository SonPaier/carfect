...
interface ProtocolViewsDialogProps {
  protocolId: string;
  open: boolean;
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            <div className="flex gap-4 text-sm text-muted-foreground bg-muted/50 rounded-lg px-3 py-2">
              <span>Wyświetlenia: <strong className="text-foreground">{totalViews}</strong></span>
              <span>Łączny czas: <strong className="text-foreground">{formatDuration(totalSeconds)}</strong></span>
            </div>
          ...
          <p className="text-sm text-muted-foreground py-4">Brak danych o oglądaniu</p>
        )}
      </DialogContent>
    </Dialog>
  );
...
export default ProtocolViewsDialog;
