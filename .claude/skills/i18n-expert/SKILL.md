---
name: i18n-expert
description: This skill should be used when setting up, auditing, or enforcing internationalization/localization in UI codebases (React/TS, i18next or similar, JSON locales), including installing/configuring the i18n framework, replacing hard-coded strings, ensuring en-US/pl-PL coverage, mapping error codes to localized messages, and validating key parity, pluralization, and formatting.
---
...
Deliver a complete i18n setup + audit pass: configure the i18n framework, replace user-facing strings with keys, ensure locale parity, and validate pluralization/formatting for en-US and pl-PL.
...
- Target locales (default: en-US + pl-PL).
...
- Confirm locales; default to en-US + pl-PL when specified.
...
- Scan all `t('key')` and `t("key")` usages across source files.
- Compare keys against all locale JSON files.
...
- For plurals use `t('key', { count })` + `_one/_other/_few/_many` keys (Polish has complex plural rules).
...
- Add missing keys in all locales.
...
- Polish has complex pluralization: one, few (2-4), many (5+), other.
- Always use `_one/_few/_many/_other` suffix pattern for Polish plurals.
- Polish diacritics (ą, ę, ó, ś, ź, ż, ł, ć, ń) MUST be preserved — never strip them.
- Date formats: DD.MM.YYYY (not MM/DD/YYYY).
- Currency: PLN with comma as decimal separator (e.g., 1 234,56 zł).
- Number formatting: space as thousands separator, comma as decimal.
...
- Some technical/brand terms should remain untranslated (e.g., Carfect, ARM-CAR, detailing).
...
- Plurals and formatting verified in all locales.
- Fallback locale configured.
- Polish diacritics correctly rendered everywhere.
