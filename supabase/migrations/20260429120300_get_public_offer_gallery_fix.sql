-- Drop the unused 1-arg overload created by the previous migration and
-- update the 2-arg signature (called by the frontend) to return gallery_photos.

DROP FUNCTION IF EXISTS public.get_public_offer(text);

CREATE OR REPLACE FUNCTION public.get_public_offer(p_token text, p_skip_mark_viewed boolean DEFAULT false)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(
    ...
    ) ORDER BY opp.sort_order, p.created_at
  ), '[]'::jsonb) INTO gallery_photos
  FROM offer_portfolio_photos opp
  JOIN instance_portfolio_photos p ON p.id = opp.photo_id
  WHERE opp.offer_id = offer_row.id;
  ...
  IF NOT p_skip_mark_viewed AND offer_row.status = 'sent' THEN
...
GRANT EXECUTE ON FUNCTION public.get_public_offer(text, boolean) TO anon;
GRANT EXECUTE ON FUNCTION public.get_public_offer(text, boolean) TO authenticated;
