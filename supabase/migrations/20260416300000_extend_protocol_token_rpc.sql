-- Extend get_protocol_by_token to also return protocol_config and custom_field_definitions
-- so the public protocol view can render custom fields and consent clauses
...
BEGIN
  ...
  -- Fetch protocol settings config for this instance + protocol type
  ...
  -- Fetch custom field definitions for this instance + protocol context
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'id', cfd.id,
