...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (windowNumber !== undefined && !HOURLY_WINDOWS[windowNumber]) {
      console.warn(`Unknown window number: ${windowNumber}. Valid windows: ${Object.keys(HOURLY_WINDOWS).join(', ')}`);
    }
    ...
    console.log(`Summary: sent=${sentCount}, skippedClaimed=${skippedClaimed}, skippedWindow=${skippedWindow}`);
