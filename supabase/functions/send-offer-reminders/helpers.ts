...
export function buildReminderEmailHtml(params: {
  ...
  const logoHtml = instanceLogoUrl
    ? `<div style="text-align:center;padding:30px 0 20px;">
        <img src="${instanceLogoUrl}" alt="${instanceName}" style="max-height:60px;max-width:200px;" />
      </div>`
    : `<div style="text-align:center;padding:30px 0 20px;">
        <h1 style="font-family:'Inter',Arial,sans-serif;font-size:22px;font-weight:700;color:#111;margin:0;">${instanceName}</h1>
      </div>`;

  const phoneHtml = instancePhone
    ? `<span style="margin:0 8px;"><a href="tel:${instancePhone}" style="color:#555;text-decoration:none;">${instancePhone}</a></span>`
    : '';
...
<tr><td style="padding:24px 12px 8px;text-align:center;">
  <p style="margin:0 0 6px;font-size:14px;color:#555;font-weight:600;">${instanceName}</p>
