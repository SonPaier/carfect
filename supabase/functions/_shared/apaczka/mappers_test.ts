...
Deno.test("APM-002: tuba maps to RURA with diameter as dimension2, height=0", () => {
  const items = mapPackagesToShipmentItems([tubaPackage]);
  assertEquals(items.length, 1);
  assertEquals(items[0].shipment_type_code, "RURA");
  assertEquals(items[0].dimension1, 100);
  assertEquals(items[0].dimension2, 15);
  assertEquals(items[0].dimension3, 0);
});
...
Deno.test("APM-004: maps full order without COD for transfer payment", () => {
  ...
  assertEquals(result.shipment_value, 15050); // 150.50 * 100
...
Deno.test("APM-005: includes COD when payment_method is cod", () => {
  const result = mapOrderToApaczkaRequest({
    order: { ...mockOrder, payment_method: "cod", total_gross: 200 },
    customer: mockCustomer,
    senderAddress: mockSender,
    packages: [kartonPackage],
    serviceId: 1,
  ...
  assertEquals(result.cod!.amount, 20000); // 200 * 100
...
Deno.test("APV-002: fails when sender address is missing", () => {
  ...
  assertEquals(result.errors.some((e) => e.includes("nadawcy")), true);
});
...
Deno.test("APV-003: fails when customer shipping address is incomplete", () => {
  ...
  assertEquals(result.valid, false);
  assertEquals(result.errors.some((e) => e.includes("odbiorcy")), true);
});
...
Deno.test("APV-004: fails when no shipping packages", () => {
  ...
  assertEquals(result.valid, false);
  assertEquals(result.errors.some((e) => e.includes("shipping")), true);
});
...
Deno.test("APV-005: fails for COD without valid bank account", () => {
  ...
  assertEquals(result.valid, false);
  assertEquals(result.errors.some((e) => e.includes("konta")), true);
});
...
Deno.test("APV-006: passes for COD with valid 26-digit bank account", () => {
  const result = validateShippingData({
    customer: mockCustomer,
    packages: [kartonPackage],
    order: { ...mockOrder, payment_method: "cod", bank_account_number: "12345678901234567890123456" },
    senderAddress: mockSender,
  });
...
Deno.test("APV-007: fails when package has no dimensions", () => {
  ...
  assertEquals(result.valid, false);
  assertEquals(result.errors.some((e) => e.includes("wymiar")), true);
});
