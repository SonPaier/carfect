...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (existingAdmin && existingAdmin.length > 0) {
      return new Response(JSON.stringify({ error: 'Admin for this instance already exists.' }), {
        status: 403,
    ...
    // Seed demo data (customers, reservations, offers, protocols) from armcar
    const { error: seedError } = await supabaseAdmin.rpc('seed_demo_data', {
      _instance_id: instanceId,
    });
    ...
    if (seedError) {
      ...
      console.log(`Demo data seeded for instance ${instanceId}`);
    }
